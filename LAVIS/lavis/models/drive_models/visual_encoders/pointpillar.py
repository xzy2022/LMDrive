...
except Exception as exc:  # pragma: no cover - depends on host binary ABI
    scatter_max = None
    scatter_mean = None
...
def _require_torch_scatter():
    if _TORCH_SCATTER_IMPORT_ERROR is not None:
        raise RuntimeError(
            "torch_scatter is required for LMDrive LiDAR inference. "
            "The package is installed but could not be imported in this environment."
...
class PointPillarNet(nn.Module):
    ...
    def decorate(self, points, unique_coords, inverse_indices):
        ...
        dtype = points.dtype
        x_centers = (
            unique_coords[inverse_indices][:, 2:3].to(dtype) / self.pixels_per_meter
            + self.min_x
        )
        y_centers = (
            unique_coords[inverse_indices][:, 1:2].to(dtype) / self.pixels_per_meter
            + self.min_y
        )
        ...
        return torch.cat([points, points_cluster, points_xp, points_yp], dim=-1)
    ...
    def grid_locations(self, points):
        ...
        coords = (
            points[:, [0, 1]]
            - torch.tensor([self.min_x, self.min_y], device=points.device)
    ...
    def forward(self, lidar_list, num_points):
        ...
        with torch.no_grad():
            ...
            for batch_id, points in enumerate(lidar_list):
                ...
                grid_byx = torch.nn.functional.pad(
                    grid_yx, (1, 0), mode="constant", value=batch_id
                )
